...
namespace Combodo\iTop\MFATotp\QRCode;
...
// URL: https://github.com/kazuhikoarase/qrcode-generator/blob/master/php/qrcode.php
//
...
class QRCode
{
	public const QR_PAD0 = 0xEC;
	public const QR_PAD1 = 0x11;
	...
	{
		$this->typeNumber = 1;
		$this->errorCorrectLevel = QR_ERROR_CORRECT_LEVEL_M;
		$this->qrDataList = [];
		$this->bgColor = '#ffffff';
		$this->fgColor = '#000000';
	}
	...
	public function addData($data, $mode = 0)
	{
		...
		switch ($mode) {
			...
			default:
				trigger_error("mode:$mode", E_USER_WARNING);
		}
	}
	...
	public static function getMinimumQRCode($data, $errorCorrectLevel): QRCode
	{
	...
	public function printHTML($size = '2px')
	{
		...
		for ($r = 0; $r < $this->getModuleCount(); $r++) {
			...
			for ($c = 0; $c < $this->getModuleCount(); $c++) {
				$color = $this->isDark($r, $c) ? $this->fgColor : $this->bgColor;
	...
	public function getImageSVG($size = 2, $iBorder = 2, $iImgWidth = 300): string
	{
		$iOverallSize = $this->getModuleCount() + (2 * $iBorder);
		$width = $iOverallSize * $size;
		$height = $width;
		$sImage = '<svg width="'.$iImgWidth.'" height="'.$iImgWidth.'" viewBox="0 0 '.$width.' '.$height.'" xmlns="http://www.w3.org/2000/svg">';
		for ($r = 0; $r < $iBorder; $r++) {
			for ($c = 0; $c < $iOverallSize; $c++) {
				$sImage .= $this->getSVGDot($c, $size, $r, $this->bgColor);
			}
		}
		for ($r = 0; $r < $this->getModuleCount(); $r++) {
			for ($c = 0; $c < $iBorder; $c++) {
				$sImage .= $this->getSVGDot($c, $size, $r + $iBorder, $this->bgColor);
			}
			for ($c = 0; $c < $this->getModuleCount(); $c++) {
				$color = $this->isDark($r, $c) ? $this->fgColor : $this->bgColor;
				$sImage .= $this->getSVGDot($c + $iBorder, $size, $r + $iBorder, $color);
			}
			for ($c = 0; $c < $iBorder; $c++) {
				$sImage .= $this->getSVGDot($this->getModuleCount() + $c + $iBorder, $size, $r + $iBorder, $this->bgColor);
			}
		}
		for ($r = 0; $r < $iBorder; $r++) {
			for ($c = 0; $c < $iOverallSize; $c++) {
				$sImage .= $this->getSVGDot($c, $size, $this->getModuleCount() + $r + $iBorder, $this->bgColor);
			}
		}
	 ...
	 * @return string
	 */
	private function getSVGDot(int $c, $size, int $r, string $color): string
	{
...
class QRUtil
{
	...
	public static function getMaxLength($typeNumber, $mode, $errorCorrectLevel)
	{
		...
		switch ($errorCorrectLevel) {
			...
			case QR_ERROR_CORRECT_LEVEL_H:
				...
				break;
			default:
				trigger_error("e:$errorCorrectLevel", E_USER_WARNING);
		}
		...
		switch ($mode) {
			...
			case QR_MODE_KANJI:
				...
				break;
			default:
				trigger_error("m:$mode", E_USER_WARNING);
		}
	...
	public static function getMask($maskPattern, $i, $j)
	{
		...
		switch ($maskPattern) {
			...
			default:
				trigger_error("mask:$maskPattern", E_USER_WARNING);
		}
	}
...
class QRRSBlock
{
	...
	public static function getRsBlockTable($typeNumber, $errorCorrectLevel)
	{
		...
		switch ($errorCorrectLevel) {
			...
			default:
				trigger_error("tn:$typeNumber/ecl:$errorCorrectLevel", E_USER_WARNING);
		}
	}
}
...
class QRNumber extends QRData
{
	...
	public static function parseIntAt($c)
	{
		...
		trigger_error("illegal char : $c", E_USER_WARNING);
	}
}
	...
	public function write(&$buffer)
	{
		...
		while ($i + 1 < strlen($data)) {
			...
			if (0x8140 <= $c && $c <= 0x9FFC) {
				...
				if (0xE040 <= $c && $c <= 0xEBBF) {
					$c -= 0xC140;
				} else {
					trigger_error('illegal char at '.($i + 1)."/$c", E_USER_WARNING);
				}
			}
		...
		if ($i < strlen($data)) {
			trigger_error('illegal char at '.($i + 1), E_USER_WARNING);
		}
	}
		...
		if (1 <= $type && $type < 10) {
			...
			switch ($this->mode) {
				...
				case QR_MODE_KANJI:
					return 8;
				default:
					trigger_error("mode:$this->mode", E_USER_WARNING);
			}
...
class QRMath
{
	...
	public static function glog($n)
	{
		...
		if ($n < 1) {
			trigger_error("log($n)", E_USER_WARNING);
		}
